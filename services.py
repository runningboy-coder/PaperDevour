...
SAVE_PATH = os.getenv("SAVE_PATH", "papers")
MAX_RESULTS = 5
...
class AnalysisService:
    ...
    @classmethod
    def set_user_api_key(cls, key):
        if key and key.strip():
            cls.client.api_key = key
        else:
            cls.client.api_key = DEEPSEEK_API_KEY # Fallback to global key
...
class ArxivService:
    ...
    def search_raw(query):
        ...
        for r in client.results(search):
            ...
            results.append({
                "entry_id": r.entry_id, "title": r.title, "summary": r.summary,
                "authors": [a.name for a in r.authors], "published": r.published.strftime('%Y-%m-%d'),
                "pdf_url": r.pdf_url, "is_imported": exists is not None
            })
        return results

    @staticmethod
    def process_and_save_paper(paper, user_id): # *** 核心修正：接收 user_id 参数 ***
            ...
            with tarfile.open(source_path, 'r:gz') as tar:
                for member in tar.getmembers():
                    if member.isfile() and member.name.lower().endswith(('.png', '.jpg', '.jpeg', '.gif')):
                        ...
                        relative_path = f"{paper_folder_name}/images/{filename}"
        ...
        new_article = Article(
            entry_id=paper.entry_id, title=paper.title, published=paper.published.replace(tzinfo=None),
            pdf_url=paper.pdf_url, original_summary=paper.summary, local_path=paper_folder_path,
            authors=authors_in_db, image_paths=image_paths,
            user_id=user_id # *** 核心修正：将文章与使用者关联 ***
        )
        db.session.add(new_article)
        db.session.commit()
        print(f"Saved new article for user {user_id}: {new_article.title}")
        return new_article
...
def fetch_and_process_for_user(user):
    print(f"Running on-demand fetch job for user: {user.username}")
    keywords = [k.keyword for k in user.keywords]
    if not keywords:
        print(f"No keywords configured for user {user.username}. Skipping fetch.")
        return
    ...
    for paper in client.results(search):
        article_in_db = ArxivService.process_and_save_paper(paper, user.id)
        if article_in_db:
            AnalysisService.set_user_api_key(user.api_key)
            analyze_and_store_article(article_in_db)
    print(f"Job finished for user: {user.username}")

def batch_import_and_process(entry_ids, user_id):
    print(f"Starting batch import for {len(entry_ids)} articles for user {user_id}.")
    paper_ids = [eid.split('/abs/')[-1] for eid in entry_ids if '/abs/' in eid]
    client = arxiv.Client()
    search = arxiv.Search(id_list=paper_ids)
    
    user = User.query.get(user_id) # 获取使用者物件以使用其 API Key
    for paper in client.results(search):
        article_in_db = ArxivService.process_and_save_paper(paper, user_id)
        if article_in_db:
            if user: AnalysisService.set_user_api_key(user.api_key)
            analyze_and_store_article(article_in_db)
